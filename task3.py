...
class Salary:
    ...
    def convert_rub(self) -> float:
        """
        Конвертирует среднее значение зарплаты в рубли
...
class Input:
    """
    Класс, принимающий входные данные
    ...
    def csv_parser(self):
        """
        Парсит csv-файл с вакансиями в массив объектов класса Vacancy
    ...
    def get_vacancy(self, row: []) -> Vacancy:
        """
        Преобразовывает массив полей вакансии в объект класса Vacancy
...
class GraphData:
    ...
    def add_data_from_vacancy(self, vacancy: Vacancy):
        ...
        salary = vacancy.salary.convert_rub()
...
class PngReport:
    ...
    def get_salary_graph(self):
        """
        Генерирует png-файл со статистикой по вакансиям в виде диаграмм
        :return: png-файл
        """
        fig = plt.figure()
    ...
    def add_bar_subplot(cls, fig: figure, title: str, width: int, full_dict: dict, x1_label: str,  axis, subplot_type="", prof_dict={}, x2_label=""):
        """
         ...
         :type x2_label: str
         :return: на область добавлена диаграмма
         """
...
class ExcelReport:
    """
    Класс для создания отчета в виде Excel-таблицы
    ...
    def generate_excel(self, dicts: list):
        ...
        """
        wb = Workbook()
        self.generate_years_sheet(wb, dicts[0], dicts[1], dicts[2], dicts[3])
        self.generate_areas_sheet(wb, dicts[4], dicts[5])
        ws = wb.worksheets[0]
        wb.remove(ws)
        wb.save("report.xlsx")
    ...
    def generate_years_sheet(self, wb: Workbook, salaries: dict, prof_salaries: dict, counts: dict, prof_counts: dict) -> worksheet.Worksheet:
        ...
        """
        years_list = wb.create_sheet("Статистика по годам")
        years_list['A1'] = 'Год'
        years_list['B1'] = 'Средняя зарплата'
        years_list['C1'] = f'Средняя зарплата - {input_set.profession}'
        years_list['D1'] = 'Количество вакансий'
        years_list['E1'] = f'Количество вакансий - {input_set.profession}'
        column_cells = [years_list["A1"], years_list['B1'], years_list['C1'], years_list['D1'], years_list['E1']]
        for cell in column_cells:
            cell.border = self.border
            cell.font = self.font
        for x in range(len(salaries)):
            years_list[f'A{x + 2}'] = list(salaries.keys())[x]
            years_list[f'B{x + 2}'] = list(salaries.values())[x]
            years_list[f'C{x + 2}'] = list(prof_salaries.values())[x]
            years_list[f'D{x + 2}'] = list(counts.values())[x]
            years_list[f'E{x + 2}'] = list(prof_counts.values())[x]
            years_list[f'A{x + 2}'].border = self.border
            years_list[f'B{x + 2}'].border = self.border
            years_list[f'C{x + 2}'].border = self.border
            years_list[f'D{x + 2}'].border = self.border
            years_list[f'E{x + 2}'].border = self.border
        dim_holder = DimensionHolder(worksheet=years_list)

        for col in range(years_list.min_column, years_list.max_column + 1):
            dim_holder[get_column_letter(col)] = ColumnDimension(years_list, min=col, max=col, width=20)

        years_list.column_dimensions = dim_holder
    ...
    def generate_areas_sheet(self, wb: Workbook, salaries: dict, counts: dict) -> worksheet.Worksheet:
        ...
        areas_list = wb.create_sheet("Статистика по городам")
        column_cells = [areas_list['A1'], areas_list['B1'], areas_list['D1'], areas_list['E1']]
        for cell in column_cells:
            cell.border = self.border
            cell.font = self.font
        areas_list['A1'] = "Город"
        areas_list['B1'] = "Уровень зарплат"
        areas_list['D1'] = "Город"
        areas_list['E1'] = "Доля вакансий"
        for x in range(10):
            areas_list[f'A{x + 2}'] = list(salaries.keys())[x]
            areas_list[f'B{x + 2}'] = list(salaries.values())[x]
            areas_list[f'D{x + 2}'] = list(counts.keys())[x]
            areas_list[f'E{x + 2}'] = list(counts.values())[x]
            areas_list[f'A{x + 2}'].border = self.border
            areas_list[f'B{x + 2}'].border = self.border
            areas_list[f'D{x + 2}'].border = self.border
            areas_list[f'E{x + 2}'].border = self.border
        dim_holder = DimensionHolder(worksheet=areas_list)

        for col in range(areas_list.min_column, areas_list.max_column + 1):
            dim_holder[get_column_letter(col)] = ColumnDimension(areas_list, min=col, max=col, width=20)

        areas_list.column_dimensions = dim_holder
        return areas_list
