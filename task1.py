...
class Salary:
    ...
    def convert_rub(self) -> float:
        value = float(self.currency_rub[self.salary_currency])
...
class ExcelReport:
    ...
    def generate_excel(self, dicts: list):
        wb = Workbook()
        years_list = self.generate_years_list(wb, dicts[0], dicts[1], dicts[2], dicts[3])
        areas_list = self.generate_areas_list(wb, dicts[4], dicts[5])
        ws = wb.worksheets[0]
        wb.remove(ws)
        wb.save("report.xlsx")

    def generate_years_list(self, wb: Workbook, salaries: dict, prof_salaries: dict, counts: dict, prof_counts: dict) -> worksheet.Worksheet:
    ...
    def generate_areas_list(self, wb: Workbook, salaries: dict, counts: dict) -> worksheet.Worksheet:
